 ...
 * 3，一个用户可以属于多个用户组(md_admin_group_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(md_admin_group 定义了用户组权限)
 *
 * 4，支持规则表达式。
 *   在 md_admin_rule 表中定义一条规则时，如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过。
 */
...
-- ----------------------------
 DROP TABLE IF EXISTS `md_admin_rule`;
CREATE TABLE `md_admin_rule` (
...
-- ----------------------------
 DROP TABLE IF EXISTS `md_admin_group`;
CREATE TABLE `md_admin_group` (
    `id` mediumint(8) unsigned NOT NULL AUTO_INCREMENT,
...
-- ----------------------------
-- md_admin_group_access 用户组明细表
-- uid:用户id，group_id：用户组id
-- ----------------------------
DROP TABLE IF EXISTS `md_admin_group_access`;
CREATE TABLE `md_admin_group_access` (
